...
class TestWhisperCppFileTranscription:
    def test_jfk_transcription_contains_expected_text(self, jfk_wav, wcpp_config):
        ...
        result = transcribe_casual(jfk_wav, wcpp_config, raise_on_error=True)
    ...
    def test_language_hint_accepted(self, jfk_wav, wcpp_config):
        ...
        result = transcribe_casual(
            jfk_wav, wcpp_config, raise_on_error=True, language="en"
        )
        assert result.strip()

    def test_direct_wc_transcribe(self, jfk_wav, wcpp_config):
        """Call the whisper_cpp backend directly, bypassing transcribe_casual."""
        # Extract whisper_cpp config from the new asr_backends structure
        wcpp_def = wcpp_config["asr_backends"]["wcpp"]
        cfg = WhisperCppConfig(
            binary=wcpp_def["binary"],
            model=wcpp_def["model"],
            threads=wcpp_def.get("threads", 4),
        )
