...
This is the main CLI entry point for kaiku.
"""
...
def _test_diarization(config: Config) -> None:
    ...
    if importlib.util.find_spec("whisperx") is None:
        warning("whisperx: not installed  →  pip install kaiku[diarize]")
...
def process_recording(config: Config):
    ...
    if config.recorder.device.mock_source:
        ...
        t0 = time.time()
        audio_data, rec_sr = record_audio(config.recorder.device, config.recorder.name)
        duration = get_audio_duration(audio_data, rec_sr)
        if duration < 0.1:
...
def _build_parser() -> argparse.ArgumentParser:
    """Build the argument parser for kaiku."""
...
""",
    ...
    vad_group.add_argument(
        ...
        help=(
            ...
            "Requires sherpa-onnx: pip install kaiku[vad]."
        ),
    )
    vad_group.add_argument(
        "--interval",
